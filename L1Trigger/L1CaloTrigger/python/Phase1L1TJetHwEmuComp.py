...
for hwTx in range(0,nTxFiles):
    with open("datFull/" + str(hwTx) + "/tx_summary.txt", "r") as inFile:
        frameIt = -1
        for line in inFile:
            if('1v' in line):
                frameIt += 1
...
with open("emuoutMBD.txt", "r") as inFile:
    for line in inFile:
        if " " in line:
            if(nEm>=nHw):
                break
...
axs[1,0].scatter(meansPt,  [(hw/em) if em>0 else 0 for hw,em in zip(nPtHw,nPtEm)] ,  c='#000000', linewidths=0.5, s=15, zorder=1)
axs[1,1].scatter(meansEta, [(hw/em) if em>0 else 0 for hw,em in zip(nEtaHw,nEtaEm)], c='#000000', linewidths=0.5, s=15, zorder=1)
axs[1,2].scatter(meansPhi, [(hw/em) if em>0 else 0 for hw,em in zip(nPhiHw,nPhiEm)], c='#000000', linewidths=0.5, s=15, zorder=1)
...
plt.savefig('hwEmuCompPtEtaPhi.pdf', bbox_inches='tight')
plt.show()
